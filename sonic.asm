...
RomEndLoc:	dc.l EndOfRom-1		; End address of ROM
		...
		dc.l v_ram_end&$FFFFFF		; End address of RAM
		if EnableSRAM=1
...
VDPSetupGame:
		...
		moveq	#(VDPSetupArray_end-VDPSetupArray)/4-1,d7
...
.wait2:
		...
		move.w	#$8F02,(a5)
		fillVRAMsrcdefined	0,$FFF,vram_fg ; clear foreground namespace
...
.mainloop:
		move.b	#id_VB_0E,(v_vbla_routine).w
		bsr.w	WaitForVBla
...
GM_Sega:
		...
		copyTilemap	v_128x128_end,$E510,$17,7
		copyTilemap	v_128x128_end+$180,$C000,$27,$1B

		tst.b   (v_megadrive).w	; is console Japanese?
		bmi.s   .loadpal
		copyTilemap	v_128x128_end+$A40,$C53A,2,1 ; hide "TM" with a white rectangle
...
Sega_WaitPal:
		move.b	#id_VB_02,(v_vbla_routine).w
		bsr.w	WaitForVBla
		bsr.w	PalCycle_Sega
		bne.s	Sega_WaitPal
		...
		move.b	#id_VB_10,(v_vbla_routine).w
		bsr.w	WaitForVBla
...
GM_Title:
		...
		copyTilemap	v_128x128_end,$C000,$27,$1B

		clearRAMopt v_pal_dry_dup,v_pal_dry_dup+16*4*2
...
Tit_LoadText:
	...
	if DebuggingMode
		...
		copyTilemap	v_128x128_end,$C208,$21,$15
	...
	if DebuggingMode
		...
		clearRAMopt v_sonicteam,v_sonicteam+object_size
...
Tit_MainLoop:
		move.b	#id_VB_04,(v_vbla_routine).w
		bsr.w	WaitForVBla
		jsr	(ExecuteObjects).l
		bsr.w	DeformLayers
		jsr	(BuildSprites).l
...
Level_ClrRam:
		clearRAM v_objspace,v_objend
		clearRAMopt v_misc_variables,v_misc_variables_end
		clearRAMopt v_levelvariables,v_levelvariables_end
		clearRAMopt v_timingandscreenvariables,v_timingandscreenvariables_end
...
Level_TtlCardLoop:
		move.b	#id_VB_0A,(v_vbla_routine).w
		bsr.w	WaitForVBla
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		bsr.w	RunPLC
...
Level_DelayLoop:
		move.b	#id_VB_06,(v_vbla_routine).w
		bsr.w	WaitForVBla
...
SS_WaitForDMA:
		...
		clearRAM v_objspace,v_objend
		clearRAMopt v_levelvariables,v_levelvariables_end
		clearRAMopt v_timingvariables,v_timingvariables_end
		clearRAMopt v_ngfx_buffer,v_ngfx_buffer_end
...
SS_MainLoop:
		bsr.w	PauseGame
		move.b	#id_VB_08,(v_vbla_routine).w
		bsr.w	WaitForVBla
		bsr.w	MoveSonicInDemo
...
SS_FinLoop:
		move.b	#id_VB_12,(v_vbla_routine).w
		bsr.w	WaitForVBla
		bsr.w	MoveSonicInDemo
		move.w	(v_jpadhold1).w,(v_jpadhold2).w
		jsr	(ExecuteObjects).l
...
loc_491C:
		...
		move.w	#$4000,d0
		bsr.w	EniDec
		copyTilemap	v_ssbuffer1,$C000,$3F,$1F
		copyTilemap_end	v_ssbuffer1,$D000,$3F,$3F
...
End_LoadData:
		...
		lea	(v_128x128_end+$1000).w,a1 ; RAM address to buffer the patterns
		bsr.w	KosPlusDec
		moveq	#palid_Sonic,d0
		bsr.w	PalLoad1	; load Sonic's palette
		moveq	#bgm_Ending,d0
...
LevelDataLoad:
		...
		move.l	a2,-(sp)
		movea.l	(a2)+,a0
		lea	(v_128x128).l,a1	; RAM address for art
		bsr.w	KosPlusDecVRAM
...
loc_8486:
		...
		bset	#5,obRender(a0)
		move.b	obID(a0),d2
		move.b	obRender(a0),d3
		move.w	obGfx(a0),d4
		move.w	obPriority(a0),d5
		move.b	obActWid(a0),d6
		movea.l	a0,a1
		bra.s	loc_84B2
; ===========================================================================
...
loc_84F2:
		...
		bra.w	PlaySound	; play collapsing sound
; ===========================================================================
; ---------------------------------------------------------------------------
...
M_Got_TBonus:	spriteHeader
	...
	spritePiece	$28, -8, 4, 2, $7F0, 1, 1, 3, 1
...
Blk16_GHZ:	binclude	"map16/GHZ.unc"
		even
Kos_GHZ_1st:	binclude	"artkosp/8x8 - GHZ1.kosp"	; GHZ primary patterns
		even
Kos_GHZ_2nd:	binclude	"artkosp/8x8 - GHZ2.kosp"	; GHZ secondary patterns
		even
Blk128_GHZ:	binclude	"map128/GHZ.kosp"
		even
Blk16_LZ:	binclude	"map16/LZ.unc"
		even
Kos_LZ:		binclude	"artkosp/8x8 - LZ.kosp"	; LZ primary patterns
		even
Blk128_LZ:	binclude	"map128/LZ.kosp"
		even
Blk16_MZ:	binclude	"map16/MZ.unc"
		even
Kos_MZ:		binclude	"artkosp/8x8 - MZ.kosp"	; MZ primary patterns
		even
Blk128_MZ:	binclude	"map128/MZ (JP1).kosp"
		even
Blk16_SLZ:	binclude	"map16/SLZ.unc"
		even
Kos_SLZ:	binclude	"artkosp/8x8 - SLZ.kosp"	; SLZ primary patterns
		even
Blk128_SLZ:	binclude	"map128/SLZ.kosp"
		even
Blk16_SYZ:	binclude	"map16/SYZ.unc"
		even
Kos_SYZ:	binclude	"artkosp/8x8 - SYZ.kosp"	; SYZ primary patterns
		even
Blk128_SYZ:	binclude	"map128/SYZ.kosp"
		even
Blk16_SBZ:	binclude	"map16/SBZ.unc"
		even
Kos_SBZ:	binclude	"artkosp/8x8 - SBZ.kosp"	; SBZ primary patterns
		even
Blk128_SBZ:	binclude	"map128/SBZ (JP1).kosp"
		even
; ---------------------------------------------------------------------------
...
SS_4:		binclude	"sslayout/4.kosp"
		even
SS_5:		binclude	"sslayout/5 (JP1).kosp"
		even
SS_6:		binclude	"sslayout/6 (JP1).kosp"
		even
; ---------------------------------------------------------------------------
