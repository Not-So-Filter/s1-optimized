...
.gotsampleduration:
		...
		beq.s	.locret			; Return if nothing is playing
...
.timpani:
		...
		stopZ80
		move.b	DAC_sample_rate(pc,d0.w),(z80_dac_timpani_pitch).l
...
.gotnote:
		...
		bra.s	.finishTrackUpdate ; This is set to branch quicker so that space is saved.
; ===========================================================================
; loc_71D1A:
.gotduration:
		bsr.w	SetDuration
...
; sub_71DC6:
DoModulation:
		btst	#1,TrackPlaybackControl(a5)	; Is modulation active?
...
; Sound_Play:
CycleSoundQueue:
		tst.l	v_soundqueue0(a6)	; is a music or sound queued for playing?
...
PlaySoundID:
		...
		clr.b	v_sound_id(a6)		; reset	music flag
...
; Sound_E1: PlaySega:
PlaySegaSound:
		lea	SegaPCM(pc),a2	; Queue Sega PCM
		lea	(ym2612_a0).l,a3
		move.w	#(SegaPCM_End-SegaPCM)-1,d2
		stopZ80
		move.b	#$2A,(a3)
; loc_71FC4:
.pcmloop:
		move.b	(a2)+,1(a3)
		moveq	#33,d1	; waste 132 cycles (to correct the pitch)
.cycleloop:
		dbf	d1,.cycleloop
		subq.w	#1,d2
		beq.s	.return
...
Sound_PlayBGM:
		...
		bne.s	Sound_PlayBGM.bgmnot1up		; if not, branch
		tst.b	f_1up_playing(a6)	; Is a 1-up music playing?
		bne.s	Sound_PlayBGM.bgm_loadMusic	; if yes, branch
...
.backupramloop:
		...
		st.b	f_1up_playing(a6)
		clr.b	v_sndprio(a6)		; Clear priority
		bra.s	Sound_PlayBGM.bgm_loadMusic
; ---------------------------------------------------------------------------
...
; byte_71A94:
SpeedUpIndex:
		dc.b s1TempotoS3($07)	; GHZ
		dc.b s1TempotoS3($72)	; LZ
		dc.b s1TempotoS3($73)	; MZ
		dc.b s1TempotoS3($26)	; SLZ
		dc.b s1TempotoS3($15)	; SYZ
		dc.b s1TempotoS3($08)	; SBZ
		dc.b s1TempotoS3($FF)	; Invincibility
		dc.b s1TempotoS3($05)	; Extra Life
		dc.b s1TempotoS3($08)	; Special Stage
		dc.b s1TempotoS3($05)	; Title Screen
		dc.b s1TempotoS3($05)	; Ending
		dc.b s1TempotoS3($04)	; Boss
		dc.b s1TempotoS3($06)	; FZ
		dc.b s1TempotoS3($03)	; Sonic Got Through
		dc.b s1TempotoS3($13)	; Game Over
		dc.b s1TempotoS3($07)	; Continue Screen
		dc.b s1TempotoS3($33)	; Credits
		dc.b s1TempotoS3($02)	; Drowning
		dc.b s1TempotoS3($06)	; Get Emerald
		even
; ===========================================================================
; loc_72024:
Sound_PlayBGM.bgmnot1up:
		moveq	#0,d0
		move.b	d0,f_1up_playing(a6)
		move.b	d0,v_fadein_counter(a6)
		move.b	d0,v_fadeout_counter(a6)
; loc_7202C:
Sound_PlayBGM.bgm_loadMusic:
		bsr.w	InitMusicPlayback
		subq.b	#bgm__First,d7
		move.b	SpeedUpIndex(pc,d7.w),v_speeduptempo(a6)
		...
		move.b	4(a4),d4		; load tempo dividing timing
...
.sfxnext:
		...
		tst.b	v_spcsfx_fm4_track+TrackPlaybackControl(a6)	; Is special SFX being played?
...
Sound_PlaySFX:
		...
		bne.s	SFX.clearpriority	; Exit is it is
		tst.b	v_fadeout_counter(a6)	; Is music being faded out?
		bne.s	SFX.clearpriority	; Exit if it is
...
.sfx_notRing:
		...
		bne.s	SFX.clearpriority.locret	; Return if not
		st.b	f_push_playing(a6)	; Mark it as playing
; Sound_notA7:
.sfx_notPush:
...
.clearramloop:
		...
		clr.b	v_sound_id(a6)	; set music to $80 (silence)
		bsr.w	FMSilenceAll
		bra.w	PSGSilenceAll
...
TempoWait:
		...
		beq.s	.skipdelay
		add.b	d0,v_main_tempo_timeout(a6)	; Has main tempo timer expired?
		bhs.s	.skipdelay	; change this to bhs for s3k tempo, bcs for s2 tempo.
...
.gotnote:
		...
		bpl.s	.gotduration	; Branch if it is a duration
		subq.w	#1,a4		; Put byte back
		bra.s	.finishTrackUpdate ; Ditto, look back at FMDoNext.
; ===========================================================================
; loc_728A4:
.gotduration:
		bsr.w	SetDuration
...
; ===========================================================================
		bra.w	cfDetune		; $E1
; ===========================================================================
		bra.w	cfJumpReturn		; $E2
; ===========================================================================
		bra.w	cfFadeInToPrevious	; $E3
; ===========================================================================
		bra.w	cfSetTempoDivider	; $E4
; ===========================================================================
		bra.w	cfChangeFMVolume	; $E5
; ===========================================================================
		bra.w	cfHoldNote		; $E6
; ===========================================================================
		bra.w	cfNoteTimeout		; $E7
; ===========================================================================
		bra.w	cfChangeTransposition	; $E8
; ===========================================================================
		bra.w	cfSetTempo		; $E9
; ===========================================================================
		bra.w	cfSetTempoDividerAll	; $EA
; ===========================================================================
		bra.w	cfChangePSGVolume	; $EB
; ===========================================================================
		bra.w	cfClearPush		; $EC
; ===========================================================================
		bra.w	cfStopSpecialFM4	; $ED
; ===========================================================================
		bra.w	cfSetVoice		; $EE
; ===========================================================================
		bra.w	cfModulation		; $EF
; ===========================================================================
		bra.w	cfEnableModulation	; $F0
; ===========================================================================
		bra.w	cfStopTrack		; $F1
; ===========================================================================
		bra.w	cfSetPSGNoise		; $F2
; ===========================================================================
		bra.w	cfDisableModulation	; $F3
; ===========================================================================
		bra.w	cfSetPSGTone		; $F4
; ===========================================================================
		bra.w	cfJumpTo		; $F5
; ===========================================================================
		bra.w	cfRepeatAtPos		; $F6
; ===========================================================================
		bra.w	cfJumpToGosub		; $F7
; ===========================================================================
		bra.w	cfOpF8			; $F8
; ===========================================================================
; loc_72ACC:
cfPanningAMSFMS:
		...
		moveq	#$37,d0			; Retain bits 0-2, 3-4 if set
		and.b	TrackAMSFMSPan(a5),d0	; Get current AMS/FMS/panning
