...
.loop:
	...
	ld	b,pcmLoopCounter(14500,127/2)	; 7	; Sega PCM pitch
	djnz	$				; 8	; Delay loop
	...
	jr	nz,.loop			; 7	; If not, loop
						; 127
	; Two samples per 127 cycles, meaning that the second parameter
	; to pcmLoopCounter should be 127 divided by 2.
...
.nospeedshoes:
	...
	or	a				; Test it
	jr	z,zInitBGMPSG			; If zero, then don't init any
...
zPlaySound_CheckGloop:
	cp	sfx_Push	; is this the pushing sfx?
	jr	nz,.skip	; if not, skip ahead
	ld	a,(zPushPlaying)
	or	a			; Test it
	ret	nz			; If it's zero, we stop everything
	ld	a,80h
	ld	(zPushPlaying),a	; enable push playing flag
.skip:
;    if ~~OptimiseDriver
;	ld	a,c
 ;   endif
...
cfClearPush:
	xor	a				; Clear 'a'
	ld	(zPushPlaying),a	; Clear zPushPlaying
; ---------------------------------------------------------------------------
...
music_metadata macro bank,pal,label
    if bank==0
.base = MusicPoint1
    endif
    if bank==1
.base = MusicPoint2
	...
	db	(bank<<7)|(pal<<6)|((label-.base)/2)
    endm

zMusIDPtr_GHZ:		music_metadata 0,0,ptr_mus81
zMusIDPtr_LZ:		music_metadata 0,0,ptr_mus82
zMusIDPtr_MZ:		music_metadata 0,0,ptr_mus83
zMusIDPtr_SLZ:		music_metadata 0,0,ptr_mus84
zMusIDPtr_SYZ:		music_metadata 0,0,ptr_mus85
zMusIDPtr_SBZ:		music_metadata 0,0,ptr_mus86
zMusIDPtr_Invinc:	music_metadata 0,0,ptr_mus87
zMusIDPtr_1UP:		music_metadata 0,0,ptr_mus88
zMusIDPtr_SS:		music_metadata 0,0,ptr_mus89
zMusIDPtr_Title:	music_metadata 0,0,ptr_mus8A
zMusIDPtr_Ending:	music_metadata 0,0,ptr_mus8B
zMusIDPtr_Boss:		music_metadata 0,0,ptr_mus8C
zMusIDPtr_FZ:		music_metadata 0,0,ptr_mus8D
zMusIDPtr_GotThrough:	music_metadata 0,0,ptr_mus8E
zMusIDPtr_GameOver:	music_metadata 0,0,ptr_mus8F
zMusIDPtr_Continue:	music_metadata 0,0,ptr_mus90
zMusIDPtr_Credits:	music_metadata 0,0,ptr_mus91
zMusIDPtr_Drowning:	music_metadata 0,1,ptr_mus92
zMusIDPtr_Emerald:	music_metadata 0,0,ptr_mus93
zMusIDPtr__End:

; Tempo with speed shoe tempo for each song
; zbyte_1214
zSpedUpTempoTable:
	db s1TempotoS3_Z80(07h)	; GHZ
	db s1TempotoS3_Z80(72h)	; LZ
	db s1TempotoS3_Z80(73h)	; MZ
	db s1TempotoS3_Z80(26h)	; SLZ
	db s1TempotoS3_Z80(15h)	; SYZ
	db s1TempotoS3_Z80(08h)	; SBZ
	db s1TempotoS3_Z80(0FFh)	; Invincibility
	db s1TempotoS3_Z80(05h)	; Extra Life
	db s1TempotoS3_Z80(08h)	; Special Stage
	db s1TempotoS3_Z80(05h)	; Title Screen
	db s1TempotoS3_Z80(05h)	; Ending
	db s1TempotoS3_Z80(04h)	; Boss
	db s1TempotoS3_Z80(06h)	; FZ
	db s1TempotoS3_Z80(03h)	; Sonic Got Through
	db s1TempotoS3_Z80(13h)	; Game Over
	db s1TempotoS3_Z80(07h)	; Continue Screen
	db s1TempotoS3_Z80(33h)	; Credits
	db s1TempotoS3_Z80(02h)	; Drowning
	db s1TempotoS3_Z80(06h)	; Get Emerald
...
dac_sample_metadata macro label,sampleRate
	...
	dac_sample_metadata zDACPtr_Timpani, 7250	; 83h
