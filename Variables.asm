...
v_128x128:		ds.b	256*128		; 128x128 tile mappings (256-1 chunks)
v_128x128_end:
			ds.b	$2400		; unused
...
v_ngfx_buffer_end:
v_spritequeue:		ds.b	$80*7		; sprite display queue, in order of priority (in this case, there is only 0 up to 6, so multiply by 7)
v_spritequeue_end:
			ds.b	$80		; unused
...
v_gameovertext1	= v_objspace+object_size*1	; object variable space for the "GAME"/"TIME" in "GAME OVER"/"TIME OVER" text ($40 bytes)
v_gameovertext2	= v_objspace+object_size*2	; object variable space for the "OVER" in "GAME OVER"/"TIME OVER" text ($40 bytes)
...
v_soundqueue0:		= v_soundqueue_start+0	; music to play
v_soundqueue1:		= v_soundqueue_start+1	; sound to play
v_soundqueue2:		= v_soundqueue_start+2	; special sound to play
...
v_spcsfx_track_ram_end:	= v_spcsfx_track_ram+TrackSz*1
...
v_plc_ptrnemcode:	ds.w	1		; pointer for nemesis decompression code ($1502 or $150C)
...
v_levseldelay:		ds.b	1		; level select - time until change when up/down is held
			ds.b	1		; unused
v_levselitem:		ds.b	1		; level select - item selected
			ds.b	1		; unused
v_levselsound:		ds.b	1		; level select - sound selected
			ds.b	1		; unused
			ds.b	$3A		; unused
